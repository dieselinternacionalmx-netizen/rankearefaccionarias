...
import { getBusinessesByState, getCategories, getMunicipalities } from '@/lib/data';

export const metadata = { title: 'Refaccionarias en Querétaro | Directorio B2B', description: 'Refaccionarias y proveedores de autopartes en Querétaro, organizados por municipio y contacto.' };
...
export default async function QueretaroHub({ searchParams }: { searchParams?: SearchParams }) {
  const params = searchParams ? await searchParams : {};
  const q = (params.q || '').toLowerCase();
  const all = getBusinessesByState('queretaro');
  const businesses = q ? all.filter(b => `${b.publicName} ${b.category} ${b.colony} ${b.municipality}`.toLowerCase().includes(q)) : all;
  const totalPages = Math.max(1, Math.ceil(businesses.length / PAGE_SIZE));
  const page = pageFrom(params.page, totalPages);
  ...
  const municipalities = getMunicipalities('queretaro');
  const categories = getCategories('queretaro');
  ...
  return (
    <main>
      <section className="page-hero">
        <div className="wrap">
          <div className="breadcrumb"><a href="/">Inicio</a> / Querétaro</div>
          <span className="eyebrow"><MapPin size={15}/> Directorio estatal</span>
          <h1 className="h1">Refaccionarias en Querétaro</h1>
          <p className="lead">Encuentra refaccionarias, autopartes y proveedores en Querétaro. Explora negocios por municipio, categoría o ubicación.</p>
      ...
      <section className="section state-overview">
        <div className="wrap overview-grid">
          <div>
            ...
            <h2>Inventario disponible en Querétaro</h2>
      ...
      <section className="section location-section">
        <div className="wrap">
          ...
          <div className="municipios">{municipalities.map((municipality) => <a className="mun" href={`/queretaro/${municipality.slug}/`} key={municipality.slug}><span>{municipality.name}</span><b>{municipality.count}</b></a>)}</div>
        </div>
      </section>
      ...
      <section className="section type-section">
        <div className="wrap">
          ...
          <div className="type-grid">{categories.map((category) => <a className="type-link" href={`/queretaro/${category.slug}/`} key={category.slug}>{category.name}<span>{category.count}</span></a>)}</div>
        </div>
      </section>
      <section className="section">
        <div className="wrap">
          ...
          {visibleBusinesses.length ? <div className="cards">{visibleBusinesses.map(b => <BusinessCard key={b.id} business={b} />)}</div> : <div className="empty">No encontramos refaccionarias con ese filtro. Intenta buscar por municipio o nombre.</div>}
          <Pagination basePath="/queretaro/" page={page} totalPages={totalPages} q={q} />
        </div>
      </section>
    </main>
  );
